...
describe('Integration: API specification search scenario', () => {
    it('searches specs and fetches endpoint details', async () => {
        const list = await SearchApiSpecs.run({ query: 'get records', category: 'records', method: 'GET' });
        expect(Array.isArray(list)).toBe(true);
        const details = await GetApiEndpoint.run({ endpoint: '/k/v1/records.json', method: 'GET' });
